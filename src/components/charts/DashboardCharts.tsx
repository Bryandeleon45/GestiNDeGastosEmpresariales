  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
      <div className="flex items-center justify-between mb-1">
        <div>
          <h2 className="text-base font-bold text-gray-900">Gastos por Departamento</h2>
          <p className="text-xs text-gray-400 mt-0.5">Clic en una barra para ver el detalle</p>
        </div>
      ...
      <ResponsiveContainer width="100%" height={190}>
        <BarChart data={CHART_DATA} barSize={34} margin={{top:8,right:0,bottom:0,left:-10}} onClick={onBarClick}>
          ...
          <Tooltip contentStyle={{fontSize:12,borderRadius:8,border:"1px solid #E5E7EB"}} formatter={(v)=>[`Q ${Number(v).toLocaleString("es-GT")}`,"Gasto"]} cursor={{fill:GL}}/>
          <Bar dataKey="gasto" radius={[3,3,0,0]} cursor="pointer" onMouseEnter={(_:unknown,i:number)=>setHoveredBar(CHART_DATA[i].dept)} onMouseLeave={()=>setHoveredBar(null)}>
